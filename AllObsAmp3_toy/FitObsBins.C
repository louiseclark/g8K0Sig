...
//root --hsfit FitObsBins.C | tee out.log
{
  ...
  FitManager RF;
  RF.SetUp().SetOutDir("out3/");
  
  ///////////////////////////////Load Variables  
  RF.SetUp().LoadVariable("phiK0[-3.14,3.14]");
  ...
  RF.SetUp().LoadCategory("polState[Polp=1,Pol0=0,Polm=-1]");//Load a category  
  ...
  // Use these to step around in amplitude space
  ...
  /////////////////////////////Make Model Signal
  RF.SetUp().FactoryPDF("RooComponentsPDF::LinPolFit(1,{phiK0,cosX,cosY,cosZ,pol,polState},=R;alpha;cosY:B;COS2:T;alpha;cosY;COS2:SIN2;alpha;cosX;Ox:SIN2;alpha;cosZ;Oz)"); // ; means * and : means +

  RF.SetUp().LoadSpeciesPDF("LinPolFit",1);

  ////////////////////////////Make Bins
  
  // Single bin 1.48/0.09
  RF.Bins().LoadBinVar("costhK0CMS",1, 0.005, 0.165);
  ...
  // Ensure the toy data keeps the generated values in the tree
  RF.Bins().KeepBranch("genphiK0");
  RF.Bins().KeepBranch("gencosX");
  RF.Bins().KeepBranch("gencosY");
  RF.Bins().KeepBranch("gencosZ");
   
  ///////////////////////////Load Data
  //RF.Data().BootStrap(2);
  RF.LoadData("HSParticles","/w/work0/home/louise/g8K0Sig/convert_output/filepPi0_all.root");
  // need to change to file with gen values RF.LoadSimulated("HSParticles","/w/work0/home/louise/g8K0Sig/convert_output_MC/filepPi0_all.root","LinPolFit");
  
  // Load Weights - variable costh bins for each eGamma bin
  ...
  // Load Weights - Ewald Sigma bins
  //RF.Data().LoadWeights("Signal","/home/louise/g8K0Sig/code/sWeights4/out_all_pi0K0_binned/Tweights.root");
  
  // Load Weights - Ewald P bins
  //RF.Data().LoadWeights("Signal","/home/louise/g8K0Sig/code/sWeights5/out_all_pi0K0_binned/Tweights.root");


  //RF.SetMinimiser(new RooMcmcUniform2Seq(2000,50,2)); // num of steps in final tree, burn in, inverse step size
  RF.SetMinimiser(new RooMcmcSeq(2000,100,2));
  ...
  // Fits already done - only need toy to run using results file
  Here::Go(&RF);
   
  //Proof::Go(&RF,6); // set to num bins for minuit fit
  auto toy=ToyManager::GetFromFit(5,RF,"ResultsHSRooMcmcSeq.root");
  toy->SetUp().SetOutDir("out2");
  //toy->LoadData("MyModel","DataSignal.root");//need data for protodata PolState and Pol
  //toy->LoadSimulated("MyModel","MC.root","SigAsym");
  toy->LoadData("HSParticles","/w/work0/home/louise/g8K0Sig/convert_output/filepPi0_all.root");
  // change to file with gen variables toy->LoadSimulated("HSParticles","/w/work0/home/louise/g8K0Sig/convert_output_MC/filepPi0_all.root","LinPolFit");
  ...
  toy->SetMinimiser(new RooMcmcSeq(2000,100,2));
  toy->SetUp().AddFitOption(RooFit::Optimize(1));
