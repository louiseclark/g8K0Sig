...
//root --hsfit FitObsBins.C
{
  ...
  /////////////////////////////Make Model Signal
  RF.SetUp().FactoryPDF("LinPolPDF::LinPolFit(phiK0,cosX,cosY,cosZ,pol,polState,B[0.556,-1.0,1.0],T[0.248,-1.0,1.0],Ox[0.242,-1.0,1.0],Oz[-0.4755,-1.0,1.0],R[0.233,-1.0,1.0])");
  ...
  //RF.SetMinimiser(new RooMcmcUniform2Seq(1000,200,200)); // num of steps in final tree, burn in, inverse step size
  // Here::Go(&RF);
  ...
  //Proof::Go(&RF,1); // set to num bins for minuit fit
