...
Bool_t Convert::Process(Long64_t entry)
{
   ...
   *lSigma = (*lTarget + beam->P4()) - (*lpip + *lpim); 
   // try reconstructing true sigma in same way
   //*ltrueSigma = (*ltrueProton + *ltruePi0); 
   *ltrueSigma = (*lTarget + beam->P4()) - (*ltruePip + *ltruePim); 
   
   *lpi0 = (*lTarget + beam->P4()) - (*lProton + *lpip + *lpim); 
   lpi0->SetXYZM(lpi0->Px(), lpi0->Py(), lpi0->Pz(), 0.13498);
   // try reconstructing true pi0 rather than using true pi0 direct
   //*ltruePi0 = truePi0->P4();
   *ltruePi0 = (*lTarget + beam->P4()) - (*ltrueProton + *ltruePip + *ltruePim); 
   ltruePi0->SetXYZM(ltruePi0->Px(), ltruePi0->Py(), ltruePi0->Pz(), 0.13498);
