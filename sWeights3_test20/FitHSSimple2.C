...
//brufit FitHSSimple2.C
{
  ...
  ///////////////////////////////Load Variables
  RF.SetUp().LoadVariable("M_pip_pim[0.4725,0.5225]");//should be same name as variable in tree  
  RF.SetUp().SetIDBranchName("fgID");

  /////////////////////////////Make Model Signal
  //RF.SetUp().FactoryPDF("Gaussian::Signal(M_pip_pim, SIMm[0.495,0.4575,0.5375], SIMw[0.01,0.001,0.02] )"); // default signal width
  RF.SetUp().FactoryPDF("Gaussian::Signal(M_pip_pim, SIMm[0.495,0.4725,0.5225], SIMw[0.004,0.001,0.008] )"); // limited signal width
  ...
  RF.SetUp().FactoryPDF("Chebychev::BG(M_pip_pim,{a0[-0.1,-1,1],a1[0.1,-1,1]})"); // quadratic
