...
TFile* outPlots=new TFile("SWeights1PARA.root","recreate"); //File to write sPlots to
/////////////////////////////////////////////////////////////////////

void SWeighter_sWeightPARA(){
  //Set up the input chain to be analysed
   TChain* tree=new TChain("HSParticles","datachain");
   tree->Add("/home/bryanm/HSAna/Events-master/Projects/K0SigSW19/SWanaPARA.root");
...
void Add1DModel(RooWorkspace* ws){
  ...
  RooRealVar sl_mean1("Mean1"," Mean1",0.497,0.494,0.500);
  RooRealVar sl_sigma1("Sigma1","Width1",0.005,0.0001,0.02);
  ...
  RooGaussian sl1("PDF1","PDF1",*(ws->var("Mpipi")),sl_mean1,sl_sigma1);
  ...
  RooChebychev bl("BackPDF","background p.d.f.",*(ws->var("Mpipi")),RooArgList(l0)) ;//Could use higher order poly
  ...
  model.graphVizTree("fullModelPARA.dot");
...
Bool_t AddData(RooWorkspace* ws, TTree* schain){
  ...
  RooRealVar MK0("Mpipi","Mpipi",0.45,0.550) ;//Note selecting these ranges cuts on the rest of the data in the tree, so fits will only be to do within this range
   //non fit variables
 ...
 //Event ID number....note the max number of events is implied 1E12
  ...
  if( schain->GetEntryList()) {
    ...
    RooDataSet data(dataSetName=TString(schain->GetEntryList()->GetName())+"DS","Total data",RooArgSet(MK0,ID),Import(*cop));
    ...
    RooDataSet data(dataSetName,"Total data",RooArgSet(MK0,ID),Import(*cop));
    cout<<"Import data to workspace"<<endl;
    // import data into workspace
    ws->import(data);
  }  
...
RooStats::SPlot*  DoSPlot(RooWorkspace* ws){
  ...
  ws->var(TString("Sigma1"))->setConstant();
  ws->var(TString("Mean1"))->setConstant();  
  ...
  model->paramOn(frame,
		 ...
		 ShowConstants());
  frame->SetTitle("Mass distribution fit");
  frame->Draw() ;
  canvas->Modified();
  canvas->Update();
