...
//root --hsfit FitHSSimple.C
{
  ...
  //RF.SetUp().SetOutDir(pwd+"out_all_pi0_binned/Egamma1.23/");
  //RF.SetUp().SetOutDir(pwd+"out_all_pi0_binned/Egamma1.48/");
  //RF.SetUp().SetOutDir(pwd+"out_all_pi0_binned/Egamma1.73/");
  RF.SetUp().SetOutDir(pwd+"out_all_pi0_binned/Egamma1.98/");

  ///////////////////////////////Load Variables
  RF.SetUp().LoadVariable("MM_p_pip_pim[0.055,0.22]"); // baseline
  RF.SetUp().SetIDBranchName("fgID");

  /////////////////////////////Make Model Signal
  RF.SetUp().FactoryPDF("Gaussian::Signal( MM_p_pip_pim, SIMm[0.135,0.055,0.22], SIMw[0.01,0.001,0.02] )");
