...
//root --hsfit FitObsBins.C | tee out.log
{
  ...
  //auto toy=ToyManager::GetFromFit(1,RF,"ResultsHSRooMcmcSeq.root");
  //toy->SetUp().SetOutDir("outToy2");
  //toy->LoadData("HSParticles","/w/work0/home/louise/g8K0Sig/convert_output/filepPi0_all.root");//need data for protodata PolState and Pol
  //Here::Go(toy); //generate toys
  
  //toy->Fitter().SetMinimiser(new RooMcmcSeq(500,100,10));
  //Here::Go(toy->Fitter()); //fit toys (this will just use minuit unless select RooMcmcSeq)
  //toy->Summarise();
