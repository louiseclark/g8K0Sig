//Run with 
//root --hsfit FitHSAsymmetryBinsSim.C --LinPolPDF.cxx
{
  //Create THSRooFit manager 
  THSRooFit* RF=new THSRooFit("AFit");
  RF->SetOutDir("out10"); //give output directory
  //RF->SetBinnedFit();
  ...
  RF->Factory("LinPolPDF::LinPolFit(phiK0,cosX,cosY,cosZ,pol,polState,B[0.0,-1.0,1.0],T[0.0,-1.0,1.0],Ox[0.0,-1.0,1.0],Oz[0.0,-1.0,1.0],R[0.0,-1.0,1.0])");
